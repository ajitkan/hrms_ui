...
export class LeaveApplyComponent {
  ...
  constructor(private fb: FormBuilder,
    ...
    this.leaveForm.get('fromDate')?.valueChanges.subscribe((value:any) => {
      if (value && this.leaveForm.get('toDate')?.value) {
        this.fetchLeaveCount();
      }
    });

    this.leaveForm.get('toDate')?.valueChanges.subscribe((value:any) => {
  ...
  ngOnInit() {
    ...
    this.route.queryParamMap.subscribe((params:any) => {
      const dateStr = params.get('selectedDate');
      console.log('selcted date---', dateStr);

      if (dateStr) {
        this.selectedDate = new Date(dateStr);  // Convert the string back to a Date object

        // Manually format the date to avoid timezone shifts
        // const formattedDate = 
